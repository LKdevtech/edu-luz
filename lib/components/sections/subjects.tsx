...
import { type Subject, SUBJECTS } from "@/lib/config/subjects";

import { Section, SectionTitle } from "./section";

// Karta pojedynczego przedmiotu — reużywana w karuzeli (mobile) i gridzie (desktop).
function SubjectCard({ s }: { s: Subject }) {
  return (
    <div className="relative h-full overflow-hidden rounded-[4px_18px_18px_4px]">
      {s.planned && (
        <div
...
// "Czego uczymy" — karty przedmiotów. Mobile: karuzela. Desktop: grid auto-fit.
export function Subjects() {
  return (
    <Section>
      ...
      {/* Mobile: karuzela z auto-scrollem co 4s */}
      <div className="md:hidden">
        <MobileCarousel ariaLabel="Przedmioty" intervalMs={4000}>
          {SUBJECTS.map((s) => (
      ...
      <div className="hidden gap-[14px] md:grid md:[grid-template-columns:repeat(auto-fit,minmax(220px,1fr))]">
        {SUBJECTS.map((s) => (
          <SubjectCard key={s.name} s={s} />
        ))}
      </div>
