...
--
-- Przy rejestracji (Supabase Auth) ROLA jest w raw_app_meta_data (ustawiane
-- wyłącznie po stronie serwera przez admin API — użytkownik NIE może jej
-- podmienić). Imię/nazwisko/telefon idą w raw_user_meta_data (dane profilowe
-- przekazywane przez signUp). Trigger tworzy odpowiadający rekord w profiles.
--
...
-- Gdy brak 'role' w app_metadata — trigger nic nie robi (profil zakłada się ręcznie).
